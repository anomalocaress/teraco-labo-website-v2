...
### 12. キャンセル時の通知過多とカレンダー削除問題の改善 (v40 採用)
- **通知の整理**: キャンセル時に Google からの通知（ゲスト削除/イベント削除）とアプリからの通知が重複し、最大3通のメールが届いていた問題を解消。
- **カレンダー削除の確実化**: 
  - 1つのイベントに複数人いる場合は「ゲスト削除」、最後の1人の場合は「イベント削除」を厳密に使い分け、二重送信を防止。
  - Googleログインユーザーには Google カレンダー公式の通知を優先し、アプリ側からの重複メールを抑制。
- **ロジックの整理**: 通知処理を `sendNotificationToTeacher` と `sendNotificationToUser` に分離し、メンテナンス性を向上。

---

## 🛠 現在の構成
- **Frontend**: GitHub Pages (`pwa-reserve/main.js`, `pwa-reserve/index.html`) - API v40 連携済み
- **Backend**: Google Apps Script (v40 通知最適化・削除確実化版)
